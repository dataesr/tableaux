...
export default function InstrumentsForAnr({ name, participantSuperOrganizationChildrenIds = [] }: { name: string | undefined, participantSuperOrganizationChildrenIds?: any[] }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["fundings-instruments-for-anr", region, structures, yearMax, yearMin],
    queryFn: () =>
  ...
  const config = {
    comment: {
      "fr": <>{`Ce graphe présente la distribution des projets auxquels participe ${structure ? "l'établissement" : "la région"}, 
        ...
        projet (en assimilant consommation et subvention pour le PIA).${withComponents ? " Ici, le périmètre est constitué d'une \
