...
export default function InstrumentsOverTimeForAnr({ name, participantSuperOrganizationChildrenIds = [] }: { name: string | undefined, participantSuperOrganizationChildrenIds?: any[] }) {
  ...
  const { data, isLoading } = useQuery({
    queryKey: ["funding-instruments-over-time-for-anr", region, structures],
    queryFn: () =>
  ...
  const config = {
    comment: {
      "fr": <>{`Ce graphique présente l’évolution temporelle du nombre de projets ou de leurs financements associés, ventilée par 
        ...
        (en assimilant consommation et subvention pour le PIA).${withComponents ? " Ici, le périmètre est constitué d'une \
