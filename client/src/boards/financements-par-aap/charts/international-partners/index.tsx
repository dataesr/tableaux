...
export default function InternationalPartners({ name }: { name: string | undefined }) {
  ...
  const body = {
    ...
    aggregations: {
      ...
      by_international_partners_funding: {
        terms: {
          field: "co_partners_foreign_inst.keyword",
  ...
  const partnersFunding = data?.aggregations?.by_international_partners_funding?.buckets ?? [];
  ...
  funders.forEach((funder) => {
    ...
    seriesProject.push({
      color: getCssColor({ name: funder, prefix: "funder" }),
      data: partnersProject.map((partner) => partner.by_project_type.buckets.find((project) => project.key === funder)?.is_coordinator?.buckets?.find((bucket) => bucket.key === 0)?.by_unique_project?.value ?? 0),
      name: [funder, getI18nLabel(i18n, 'not-coordinator')].join(' - '),
    });
  ...
  const title = `Principaux partenaires internationaux de ${structure ? "l'établissement" : "la région"} ${name} ${getYearRangeLabel({ yearMax, yearMin })}`;
  ...
  const config = {
    comment: {
      "fr": <>{`Ce graphe montre quels partenaires internationaux collaborent le plus avec ${structure ? "l'établissement" : "la région"} 
        sur les projets financés par AAP. Les barres représentent le nombre, les financements globaux et perçus pour les projets 
        auxquels ${structure ? "l'établissement" : "la région"} participe avec chaque partenaire. ${structure ? "Le type de \
