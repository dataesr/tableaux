...
import { dbAccessibility } from "../../../services/mongo-accessibility.js";
const router = new express.Router();

// Retourne toutes les audits disponibles
router.get("/get-audits", async (req, res) => {
  try {
    const audits = await dbAccessibility.collection("audits").find({}).toArray();
    res.json(audits);
  } catch (error) {
...
router.get("/get-last-audit/:siteId", async (req, res) => {
  ...
  try {
    const audits = await dbAccessibility.collection("audits").find({ siteId }).toArray();
    if (audits.length === 0) {
...
router.get("/get-rgaa/:rgaaId", async (req, res) => {
  ...
  try {
    const rgaa = await dbAccessibility.collection("versions-rgaa").find({ rgaaId }).toArray();

    if (rgaa.length === 0) {
      // Si aucun RGAA n'est trouvé, retourne le dernier RGAA disponible
      const lastRgaa = await dbAccessibility.collection("versions-rgaa").find({}).sort({ rgaaId: -1 }).limit(1).toArray();
...
router.post("/update-tests", async (req, res) => {
  ...
  try {
    ...
    const existingAudit = await dbAccessibility.collection("audits").findOne({ siteId, rgaaId });
    ...
    if (existingAudit) {
      ...
      for (const test of tests) {
        ...
        if (existingTest) {
          ...
          await dbAccessibility.collection("audits").updateOne({ _id: existingAudit._id }, { $set: { "tests.$[elem]": updatedTest } }, { arrayFilters: [{ "elem.testId": test.testId }] });
        } else {
          // Si le test n'existe pas, l'ajouter
          const newTest = { ...test, createdAt: new Date(), updatedAt: new Date() };
          await dbAccessibility.collection("audits").updateOne({ _id: existingAudit._id }, { $push: { tests: newTest } });
        }
      }
    } else {
      ...
      await dbAccessibility.collection("audits").insertOne({
        siteId,
        rgaaId,
